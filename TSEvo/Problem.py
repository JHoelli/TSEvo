...
class MultiObjectiveCounterfactuals(Problem):
    ...
    def __init__(self, model, observation,original_y,  target,reference_set,
                 neighborhood,window, backend='torch',channels=1):
    ...
    def _evaluate(self, explanation, out, *args, **kwargs):
        ...
        num_changed_features = np.count_nonzero(self.observation - explanation) / (self.observation.shape[-1] * self.observation.shape[-2])  #TODO Used to be 150 !
